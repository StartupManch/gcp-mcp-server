...
import { ToolCallArgs, ToolResponse, ExecutionResult } from '../types';
...
export class GCPToolHandlers {
  ...
  async executeGCPCode(args: ToolCallArgs): Promise<ToolResponse> {
    ...
    try {
      ...
      if (!currentProjectId) {
        throw new Error("No project selected. Please select a project first using the select-project tool.");
      }
